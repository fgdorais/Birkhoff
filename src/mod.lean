...
lemma model.axiom (i : I) : ax ⊨ ax i := λ _ _ ha, ha i

namespace model

lemma reflexivity (t : term sig) : ax ⊨ t ≡ t :=
λ _ _ _ _, rfl

lemma symmetry {{t u : term sig}} : (ax ⊨ t ≡ u) → (ax ⊨ u ≡ t) :=
λ h _ a ha val, eq.symm (h a ha val)

lemma transitivity {{t u v : term sig}} : (ax ⊨ t ≡ u) → (ax ⊨ u ≡ v) → (ax ⊨ t ≡ v) :=
λ htu huv α a ha val, eq.trans (htu a ha val) (huv a ha val)

lemma substitiution (sub : ℕ → term sig) {{t u : term sig}} :
...
lemma replacement {{sub₁ sub₂ : ℕ → term sig}} :
...
| h (term.app s ts) :=
  ...
  from tup.ext (λ i, replacement h (ts i) a ha val), 
  calc 
